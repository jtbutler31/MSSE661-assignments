...
exports.getAllBdomeals = async (req, res) => {
  ...
  const bdomeals = await query(con, ALL_BDOMEALS(req.user.id), []).catch(
    serverError(res)
  );
...
exports.updateBdomeal = async (req, res) => {
  ...
  const values = -buildValuesString(req);

  const result = await query(con, UPDATE_BDOMEAL(req.user.id, req.params.bdomealId, values)
  ).catch(serverError(res));

  if (result.affectedRows !== 1) {
  res
    .status(500)
    .json({msg: 'Unable to update bdomeal: ${req.body.name}'});
  }
    res.json(result);
