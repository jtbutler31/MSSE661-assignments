...
exports.register = async (req, res) => {
  ...
  const params = [req.body.username, passwordHash];
  ...
  const user = await query(con, GET_ME_BY_USERNAME, [req.body.username])
    ...
    if(user.length === 1) {
        ...
        const username = mysql.escape(params[0]);
        const password = mysql.escape(params[1]);

        const result = await query(con, INSERT_NEW_USER, params)
        .catch((err) => {
          res
            .status(500)
            .json({ msg: 'Could not register user. Please try again later.' });
